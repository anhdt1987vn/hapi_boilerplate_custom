...
describe('API', () => {
  ...
  it('Restricted register route should return http status 401 for anonymous user', done => {
    server.inject('/register', response => {
      expect(response.statusCode).to.equal(401);
      done();
    });
  });

  it('Restricted register route should return http status 200 for authenticated user', done => {
    var options = {
      method: 'POST',
      url: '/register',
      headers: {
